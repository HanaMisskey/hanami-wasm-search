...
async function getEmojisData() {
  ...
  console.log('misskey.io/api/emojis.jsonからデータをダウンロード中...');
...
async function runBenchmark() {
  ...
  console.log('WASM Module:', typeof wasm, wasm ? 'loaded' : 'not loaded');
  console.log('Index constructor:', typeof wasm?.Index);
  ...
  try {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      console.log('インデックス作成完了:', index ? 'success' : 'failed');
      ...
      index.add_documents(JSON.stringify(processedData));
      ...
      emojisData.emojis.forEach((emoji, idx) => {
        try {
          index.removeDocument(emoji.name);
          if (idx % 1000 === 0) {
